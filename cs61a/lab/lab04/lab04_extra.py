...
def add_song(t, song, category):
    ...
    if label(t) == category:
        ...
        return tree(label(t), [add_song(branch, song, category) for branch in branches(t)])
...
def delete(t, target):
    ...
    if label(t) == target:
        return None
    all_branches = [delete(br, target) for br in branches(t) if delete(br, target) is not None]
