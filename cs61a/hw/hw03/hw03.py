...
def lt_rat(x, y):
    ...
    False
    """
    return (denom(x)*denom(y)) * (numer(x)*denom(y) - numer(y)*denom(x)) < 0
...
def build_binary_ones(h):
    ...
    if h == 0:
        ...
        return tree('1', [build_binary_ones(h-1) for i in range(2)])
