...
def simplify(expr):
    ...
    if expr.is_leaf():
        ...
        lexpr = simplify(expr[0])
        rexpr = simplify(expr[1])

        op = expr.oper
        newExpr = Expr(op, lexpr, rexpr)
        if op == '*':
            if lexpr == ZERO or rexpr == ZERO:
                return ZERO
            if lexpr == ONE or rexpr == ONE:
                return lexpr if rexpr == ONE else rexpr
            return newExpr
        if op == '+':
            if lexpr == ZERO or rexpr == ZERO:
                return lexpr if rexpr == ZERO else rexpr
            return newExpr
        if op == '-':
            if rexpr == ZERO:
                return lexpr
...
class VendingMachine:
    ...
    def restock(self, num_items):
        assert num_items > 0, "can only restock more than zero items."
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            change_string = " and ${} change.".format(change) if change > 0 else "."
            self.stock -= 1
            self.balance = 0
            return 'Here is your {}'.format(self.product) + change_string

    def deposit(self, money):
        assert money > 0, "money should > 0"
        self.balance += money
        if self.stock <= 0:
...
def zip(*iterables):
    ...
    [2, 5, 8]
    """
    iters = [iter(ll) for ll in iterables]

    while True:
        try:
            ele = [next(it) for it in iters]
            yield list(ele)
